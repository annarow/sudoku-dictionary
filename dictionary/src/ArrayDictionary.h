...
public:
    ...
    bool add(KeyType const &key, ValueType const &value) override {
        ...
        std::cout << hashedKey<< ", " << key << std::endl;
    ...
    bool remove(KeyType const &key) override {
        // homework
        int hashedKey = hashFunc(key);
        //check for proper haskey
        if (hashedKey != key % capacity) {
            return false;
        }
        //check if entries[hashkey] is empty
        if (entries[hashedKey] == nullptr) {
            return false;
        }
        //check if entires[hashkey] == key, and make pointer and use recursive function call with ptr
    ...
    void removeEntry(Entry<KeyType, ValueType> *ptr) {
        if(ptr == nullptr){
            return;
        }
        removeEntry(ptr->next);
        delete ptr;
        }
        //then delete ptr
